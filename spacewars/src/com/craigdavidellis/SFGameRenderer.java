...
public class SFGameRenderer implements Renderer{
	...
	private SFHero player1 = new SFHero();
	...
	public void onDrawFrame(GL10 gl) {
	    ...
	    hm.addScore("Craig",240);
	    hm.addScore("Steve",300);
	    hm.addScore("Ross",220);
	...
	private void movePlayer1(GL10 gl){
		if(!player1.isDestroyed){
			...
			case SFEngine.PLAYER_BANK_LEFT_1:
				...
				gl.glPushMatrix();
				gl.glScalef(.25f, .25f, 1f);			
				if (heroBankFrames < SFEngine.PLAYER_FRAMES_BETWEEN_ANI && SFEngine.playerBankPosX > 0){
					...
					heroBankFrames += 1;
				}else if (heroBankFrames >= SFEngine.PLAYER_FRAMES_BETWEEN_ANI && SFEngine.playerBankPosX > 0){
					SFEngine.playerBankPosX -= SFEngine.PLAYER_BANK_SPEED;
					gl.glTranslatef(SFEngine.playerBankPosX, 0f, 0f);
					gl.glMatrixMode(GL10.GL_TEXTURE);
					gl.glLoadIdentity();
			...
			case SFEngine.PLAYER_BANK_RIGHT_1:
				...
				gl.glPushMatrix();
				gl.glScalef(.25f, .25f, 1f);			
				if (heroBankFrames < SFEngine.PLAYER_FRAMES_BETWEEN_ANI && SFEngine.playerBankPosX < 3){
					SFEngine.playerBankPosX += SFEngine.PLAYER_BANK_SPEED;
					gl.glTranslatef(SFEngine.playerBankPosX, 0f, 0f);
					gl.glMatrixMode(GL10.GL_TEXTURE);
					gl.glLoadIdentity();
					...
					heroBankFrames += 1;
				}else if (heroBankFrames >= SFEngine.PLAYER_FRAMES_BETWEEN_ANI && SFEngine.playerBankPosX < 3){
					gl.glTranslatef(SFEngine.playerBankPosX, 0f, 0f);
					gl.glMatrixMode(GL10.GL_TEXTURE);
					gl.glLoadIdentity();
				...
				for (int x = 0; x < SFEngine.TOTAL_INTERCEPTORS + SFEngine.TOTAL_SCOUTS + SFEngine.TOTAL_WARSHIPS - 1; x++ ){
					if(!enemies[x].isDestroyed && enemies[x].posY < 4.25 ){
						if ((playerFire[y].posY >= enemies[x].posY - 1 
							...
							score = score + 10; //Add score of 10 when enemy is hit
							...
							if (enemies[x].equals(0)){
								AlertDialog.Builder builder = new AlertDialog.Builder(null);
								builder.setMessage("Do you want to Continue?").setPositiveButton("Yes", dialogClickListener).setNegativeButton("No", dialogClickListener).show();
							}
						}
					}
				}
	...
	public void onSurfaceChanged(GL10 gl, int width, int height) {
		...
		gl.glMatrixMode(GL10.GL_PROJECTION); /**to access glOrthhof, OpenGL put into projection matrix mode*/
		gl.glLoadIdentity(); /**Default state of OpenGL*/
		
		gl.glOrthof(0f, 1f, 0f, 1f, -1f, 1f); /**Orthogonal 2D rendering x,y,z*/
